...
#define  MaxMotorSpeed   200    //自己随便设的
...
//sPID pidController( float tarSpeed ,float  curSpeed);
sPID  Fuzzifier(   sPID *  tmp);

//float  pidController( float tarSpeed ,float  curSpeed , sPID * motor);
sPID  pidController( float tarSpeed ,float  curSpeed , sPID * motor);

//void  Fuzzifier(   sPID *  fuzzyDetPID);
#endif

...
{  							  
	...
	//	else if(size==32)temp=asc2_32[num][t];	//调用2412字体
		else return;								//没有的字库
		for(t1=0;t1<8;t1++)                         //
		{			    
...
//汉字显示函数
void LCD_ShowHanzi(u16 x,u16 y,u8 num,u8 size,u8 mode)
{  							  
    u8 temp,t1,t;
	u16 y0=y;
	u8 csize=(size/8+((size%8)?1:0))*(size/2);		//得到字体一个字符对应点阵集所占的字节数	
 	//得到偏移后的值（ASCII字库是从空格开始取模，所以-' '就是对应字符的字库）
	for(t=0;t<csize;t++)                                            //32个二进制数据循环
	{   
		if(size==12)temp=asc2_1206[num][t]; 	 	//调用1206字体
		else if(size==16)temp=asc2_1608[num][t];	//调用1608字体
		else if(size==24)temp=asc2_2412[num][t];	//调用2412字体
...
{
	char Index[3];// 汉字内码索引,汉字在内存中占两个字节
	char Msk[32]; // 点阵码数据
};
//放C文件
...
{	
   {"吗",
    ...
    0x3F, 0xF0 ,0x00 ,0x00 ,0x40 ,0x10, 0x4F ,0x10 ,
    0x41 ,0x10, 0x41, 0x10, 0x41 ,0x12 ,0x7F ,0x11 ,
    0x01, 0x02, 0x01, 0xFC, 0x00, 0x00 ,0x00 ,0x00},
	 
	{"高" , 
	 0x20, 0x00, 0x20, 0x7F, 0x20, 0x40, 0x20, 0x40,
	0x2F, 0x5E, 0x29, 0x52, 0xA9, 0x52, 0x69, 0x52,
	0x29, 0x52, 0x29, 0x52, 0x2F, 0x5E, 0x20, 0x40,
	0x20, 0x41, 0x20, 0x7F, 0x20, 0x00, 0x00, 0x00},
...
void show_chinese(u16 x,u16 y,const char *p,u8 mode)
{
	...
	{
		if(!strncmp(p ,code_GB_16[wordNum].Index ,3 ))     //逐个比较目标汉字和字库里面的
		{
			//开始比较
			for(wordByte=0;wordByte<32;wordByte++)
			{
				data =code_GB_16[wordNum].Msk[wordByte];   //得到目标汉字的32个二进制数
					
				for(k=0;k<8;k++)
				   {
					if((data&0x80)==0x80)  LCD_Fast_DrawPoint(x,y,POINT_COLOR);
...
/////////////////////显示汉字字符串
void showHanzi(u16 x,u16 y,const char *p,u8 mode){
//	u8 x0=x;
//	width+=x;
//	height+=y;
	char tar[2];   //被拷贝的目标数组
...
//		 
//		LCD_Fast_DrawPoint(30 + xCounts 450 ,POINT_COLOR);        //原点（30 ，450）
//	 }
...
//[0]:0,非叠加显示;1,叠加显示.
	  ...
	  {
		   LCD_ShowxNum( 40 + (400/yMax)*(tarHeight/4)*x_scale ,25, (int)(tarHeight/4) *x_scale , 3 ,16,0);
		  drawLine(50 + (400/yMax)*(tarHeight/4)*x_scale , 48 , 50 + (400/yMax)*(tarHeight/4)*x_scale , 40);
	  }
	  
	 
	 for( ; x < counts/10 ; x ++ ){
		  
		// tmpY = (int)(450 - curData[x]*(400/ yMax) );
		 tmpY = (int)( curData[x]*(400/ yMax) ) + 50;
		 
		 // tmpX = (int)( x*(400/ xMax)) + 30;
		 tmpX = (int)( x*(750/ xMax)) + 50;
			 
		
          printf("%d ,%d \r\n" , tmpX , tmpY);
		 
	//	  LCD_Fast_DrawPoint(tmpX  , tmpY  ,POINT_COLOR);     //画小数点
	//	 LCD_ShowChar(tmpX,tmpY , '.' , 16 , 0);
		 
		  drawLine( lastY  , lastX ,  tmpY  ,tmpX);     
		 
	//	  LCD_ShowChar(tmpX,tmpY , '*' , 16 , 0);              //画*
		  lastX = tmpX;
	      lastY = tmpY; 
		 
		
	 }
//	  drawLine( 30 , 450 ,  35 ,197);
//	  drawLine( 35, 197 , 40,  365 );
//	  drawLine( 40, 365 , 45,  185 );
//	  drawLine( 45, 185 , 50,  300 );
//	  drawLine( 50, 300 , 55,  176 );
//	  drawLine( 55, 176 , 60, 255 );
	 
//	 for( x = 0 ; x < counts/2 -1 ; x ++ ){
//		  
//		  drawLine( lastX[x] , lastY[x] ,  lastX[x + 1] ,lastY[x + 1]);
//		 
//	 }
...
//	 if(x1 < x2 && y1 <= y2   )  {
//		 
//		 for(  ; x_counts < x_abs , y_counts <  y_abs; x_counts ++ , y_counts ++){
//			 
//			LCD_Fast_DrawPoint(x1 + x_abs , y1 + y_abs  ,POINT_COLOR);        
//		} 
//		 
//	 }
...
//		 for(  ; x_counts < x_abs , y_counts <  y_abs; x_counts ++ , y_counts ++){
//			 
//			LCD_Fast_DrawPoint(x1 + x_abs , y2 + y_abs  ,POINT_COLOR);        
//		} 
//		 
//	 }
...
//		 for(  ; x_counts < x_abs , y_counts <  y_abs; x_counts ++ , y_counts ++){
//			 
//			LCD_Fast_DrawPoint(x2 + x_abs , y1 + y_abs  ,POINT_COLOR);        
//		} 
//		 
//	 }
...
//		 for(  ; x_counts < x_abs , y_counts <  y_abs; x_counts ++ , y_counts ++){
//			 
//			LCD_Fast_DrawPoint(x2 + x_abs , y2 + y_abs  ,POINT_COLOR);        
//		} 
//		 
//	 }
...
//		 for(  ; x_counts < x_abs , y_counts <  y_abs; x_counts ++ , y_counts ++){
//			 
//			LCD_Fast_DrawPoint(x2  , y2 + y_abs  ,POINT_COLOR);        
//		} 	 	 
//	 }
...
//		 for(  ; x_counts < x_abs , y_counts <  y_abs; x_counts ++ , y_counts ++){
//			 
//			LCD_Fast_DrawPoint(x2 , y1 + y_abs  ,POINT_COLOR);        
//		} 
//		 
//	 }
